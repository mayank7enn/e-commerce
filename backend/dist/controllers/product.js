...
export const getLatestProducts = TryCatch(async (req, res, next) => {
    const products = await getCachedData("latest-products", () => Product.find().sort({ createdAt: -1 }).limit(5));
...
export const getAllCategories = TryCatch(async (req, res, next) => {
    const categories = await getCachedData("categories", () => Product.distinct("category"));
    return res.status(201).json({
        success: true,
        categories,
    });
...
export const getAdminProducts = TryCatch(async (req, res, next) => {
    const products = await getCachedData("all-products", () => Product.find({}));
    return res.status(201).json({
        success: true,
        products,
    });
...
export const getSingleProduct = TryCatch(async (req, res, next) => {
    ...
    const product = await getCachedData(`product-${id}`, () => Product.findById(id));
    if (!product)
